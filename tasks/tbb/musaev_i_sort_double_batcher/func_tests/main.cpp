...
TEST(musaev_i_sort_double_batcher, array) {
  ...
  // Create Task
  MusaevIlgarTbb::RadixSortDoubleBatcherSequential testRDSBSequential(RSDBDataSeq);
  ...
  // Create Task
  MusaevIlgarTbb::RadixSortDoubleBatcherTbbParallel testRDSBtbbParallel(RSDBDataTbb);
...
TEST(musaev_i_sort_double_batcher, test_random) {
  // Create data
  std::vector<double> array = MusaevIlgarTbb::randomVector(1000, -1000000, 1000000);
  // Create data
  std::vector<double> out_seq(array.size());
