...
#include "tbb/musaev_i_sort_double_batcher/include/ops_tbb.hpp"

TEST(musaev_i_sort_double_batcher, test_pipeline_run) {
  // Create data
  std::vector<double> array = MusaevIlgarTbb::randomVector(1000000, -1000000, 1000000);
  ...
  // Create Task
  auto testRSDBtbbParallel = std::make_shared<MusaevIlgarTbb::RadixSortDoubleBatcherTbbParallel>(RSDBDataTbb);
...
TEST(musaev_i_sort_double_batcher, test_task_run) {
  // Create data
  std::vector<double> array = MusaevIlgarTbb::randomVector(1000000, -1000000, 1000000);
  std::vector<double> out(array.size());
